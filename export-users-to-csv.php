...
class PP_EU_Export_Users
{
    ...
    public function load_textdomain()
    {
        ...
        load_textdomain('export-users-to-csv',
          WP_LANG_DIR . '/export-users-to-csv/export-users-to-csv-' . $locale . '.mo');
    ...
    public function filter_export_args()
    {
        ...
        <div class="eutcsv_leave_review">
            ...
            <p><?php echo __('If "Export Users to CSV" has been useful for you, please take a minute to let me know by <a href="https://wordpress.org/support/plugin/export-users-to-csv/reviews/?filter=5">leaving a great rating here</a>.',
                  'export-users-to-csv'); ?></p>
        </div>
        <?php
    }
    ...
    public function pre_user_query($user_search)
    {
        ...
        if (!empty($_GET['start_date'])) {
            $where .= $wpdb->prepare(" AND $wpdb->users.user_registered >= %s",
              date('Y-m-d', strtotime($_GET['start_date'])));
        }

        if (!empty($_GET['end_date'])) {
            $where .= $wpdb->prepare(" AND $wpdb->users.user_registered < %s",
              date('Y-m-d', strtotime('+1 month', strtotime($_GET['end_date']))));
        }
    ...
    function eutc_add_export_button()
    {
        ...
        <script type="text/javascript">
          jQuery(document).ready(function ($) {
            jQuery(jQuery(".wrap .wp-header-end")[0])
              .before('<a href="<?php echo admin_url('export.php'); ?>" class="page-title-action"><?php echo __('Export Users',
                'export-users-to-csv'); ?></a>');
          });
        </script>
        <?php
    }
